...
def load_complaints():
    if os.path.exists(COMPLAINTS_FILE):
        with open(COMPLAINTS_FILE, 'r', encoding='utf-8') as f: return json.load(f)
    return {}

def save_complaints(complaints):
    with open(COMPLAINTS_FILE, 'w', encoding='utf-8') as f: json.dump(complaints, f, ensure_ascii=False, indent=2)

def load_marriages():
    if os.path.exists(MARRIAGES_FILE):
        with open(MARRIAGES_FILE, 'r', encoding='utf-8') as f: return json.load(f)
    return {}

def save_marriages(marriages):
    with open(MARRIAGES_FILE, 'w', encoding='utf-8') as f: json.dump(marriages, f, ensure_ascii=False, indent=2)
...
def save_chats(chats):
    with open(CHATS_FILE, 'w', encoding='utf-8') as f: json.dump(chats, f, ensure_ascii=False, indent=2)
...
async def message_filter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    nsfw_words = ["порно", "porn", "xxx", "секс", "sex", "член", "пизда", "хуй", "ебать",
        "шлюха", "интим", "минет", "анус", "голые", "обнаженные", "эротика",
        "18+", "18 +", "для взрослых", "adult", "трах", "кончил", "сперма",
        "голая", "голый", "нюдс", "nudes", "nude", "слив", "сливы", "приват", "onlyfans"]
    ...
    if "http://" in message_text or "https://" in message_text or "t.me/" in message_text:
        try:
            ...
            if chat_member.status in ['administrator', 'creator']: return
        except: pass
        try:
            await update.message.delete()
...
async def admin_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("take_"):
        ...
        if complaint_id in complaints and complaints[complaint_id]["status"] == "open":
            ...
            save_chats(chats)
            try:
                await context.bot.send_message(chat_id=complaints[complaint_id]["user_id"], text="✅ Администратор взял обращение. Пишите ваш вопрос.")
            except: pass
            await query.message.edit_reply_markup(reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Завершить", callback_data=f"close_{complaint_id}")]]))
        ...
        if complaint_id in complaints:
            ...
            save_complaints(complaints)
            chats = load_chats()
            uid = str(complaints[complaint_id]["user_id"])
            aid = str(complaints[complaint_id].get("admin_id", ""))
            if uid in chats: del chats[uid]
            if aid in chats: del chats[aid]
            save_chats(chats)
            try: await context.bot.send_message(chat_id=complaints[complaint_id]["user_id"], text="✅ Обращение закрыто. Спасибо!")
            except: pass
...
async def admin_menu_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = query.data
    msgs = {"admin_ban": "Ответьте на сообщение и напишите: Бан [время]", "admin_mute": "Ответьте на сообщение и напишите: Мут [время]",
            "admin_warn": "Ответьте на сообщение и напишите: Пред [причина]", "admin_unban": "Ответьте на сообщение и напишите: Разбан",
            "admin_unmute": "Ответьте на сообщение и напишите: Размут", "admin_clear": "Напишите: Очистка [количество]"}
    if data in msgs: await query.message.reply_text(msgs[data])
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("🚫 Бан", callback_data="admin_ban"), InlineKeyboardButton("🔇 Мут", callback_data="admin_mute")],
...
async def clear_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"🧹 Удалено {len(msgs)}!")
...
async def private_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_data.get('waiting_for_complaint'):
        ...
        complaints[complaint_id] = {"user_id": user_id, "username": update.effective_user.first_name, "text": message_text, "status": "open", "admin_id": None}
        save_complaints(complaints)
    ...
    if user_id_str in chats:
        ...
        if complaint_id in complaints and complaints[complaint_id]["status"] == "in_progress":
            if is_admin(user_id) or user_id == CREATOR_ID:
                ...
                try: await context.bot.send_message(chat_id=target_id, text=f"📩 Администратор:\n{message_text}")
                except: pass
            else:
                admin_id = complaints[complaint_id].get("admin_id")
                if admin_id:
                    try: await context.bot.send_message(chat_id=admin_id, text=f"📩 {update.effective_user.first_name}:\n{message_text}")
                    except: pass
            return
    
    if not (is_admin(user_id) or user_id == CREATOR_ID):
        keyboard = [[InlineKeyboardButton("🆘 Поддержка", callback_data="support")]]
        await update.message.reply_text("Нажмите кнопку для обращения.", reply_markup=InlineKeyboardMarkup(keyboard))
# Бан
...
async def marry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    m[str(a.id)] = {"partner_id": b.id, "name": b.first_name}
    m[str(b.id)] = {"partner_id": a.id, "name": a.first_name}
    save_marriages(m)
    await update.message.reply_text(f"💒 {a.first_name} и {b.first_name} теперь в браке! 💍")

# Развод
async def divorce(update: Update, context: ContextTypes.DEFAULT_TYPE):
    u = update.effective_user; m = load_marriages()
    if str(u.id) not in m: await update.message.reply_text("❌ Вы не в браке!"); return
    pid = m[str(u.id)]["partner_id"]; pname = m[str(u.id)]["name"]
    del m[str(u.id)]
    if str(pid) in m: del m[str(pid)]
    save_marriages(m)
    await update.message.reply_text(f"💔 {u.first_name} развёлся с {pname}")
...
async def hug(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.reply_to_message: return
    a, b = update.effective_user, update.message.reply_to_message.from_user
    gifs = ["https://media.giphy.com/media/od5H3PmEG4EVq/giphy.gif", "https://media.giphy.com/media/ZQN9jsRWp1M76/giphy.gif", "https://media.giphy.com/media/wnsgren9NtITS/giphy.gif"]
    await update.message.reply_animation(animation=random.choice(gifs), caption=f"🫂 {a.first_name} обнял(а) {b.first_name}!")

# Поцеловать
async def kiss(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.reply_to_message: return
    a, b = update.effective_user, update.message.reply_to_message.from_user
    gifs = ["https://media.giphy.com/media/G3va31oEEnIkM/giphy.gif", "https://media.giphy.com/media/12VXIxKaIEarL2/giphy.gif", "https://media.giphy.com/media/jR22gdcPiOLaE/giphy.gif"]
    await update.message.reply_animation(animation=random.choice(gifs), caption=f"💋 {a.first_name} поцеловал(а) {b.first_name}!")

# Укусить
async def bite(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.reply_to_message: return
    a, b = update.effective_user, update.message.reply_to_message.from_user
    gifs = ["https://media.giphy.com/media/l4FGH2AqGg0O4LTkA/giphy.gif", "https://media.giphy.com/media/3o7TKqoVPf7K1M8T9K/giphy.gif", "https://media.giphy.com/media/xT0BKnZ6dO6FCxDtBK/giphy.gif"]
    await update.message.reply_animation(animation=random.choice(gifs), caption=f"🦷 {a.first_name} укусил(а) {b.first_name}!")
# Судные выходные
...
def main():
    ...
    application.job_queue.run_daily(send_weekend_message, time=time(hour=12, minute=0, tzinfo=MOSCOW_TZ), days=(5,), chat_id=CHAT_ID)
    application.job_queue.run_daily(send_weekend_message, time=time(hour=12, minute=0, tzinfo=MOSCOW_TZ), days=(6,), chat_id=CHAT_ID)
